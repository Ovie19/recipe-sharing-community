...
class RecipeService:
    ...
    # CRUD
    def create_recipe(self, recipe_id: RecipeCreate,user_id: UUID) -> RecipeResponse:
        recipe = Recipe.model_validate(recipe_id,update={"user_id": user_id})
    ...
    def view_recipes(self,skip: int = 0,limit: int = 100) -> List[RecipeResponse]:
        recipes = self.repository.view_all_recipes(skip=skip,limit=limit)
    ...
    def update_recipe(self,recipe_id: UUID,recipe_in: RecipeCreate) -> RecipeResponse:
        ...
        updated_recipe = self.repository.update_recipe(recipe_id,update_data)
